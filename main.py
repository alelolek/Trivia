...
# appending instances to list
preguntas.append(
    Pregunta(1, '1) ¿Desde cuándo podemos copiar y pegar?', 'a', 1))
preguntas.append(
    Pregunta(2, '2) ¿Recuerdas cuál fue el primer navegador de la historia?',
             'c', 1))
preguntas.append(
    Pregunta(3, '3) En qué año se desarrolló el lenguaje de programación C?',
             'd', 1))

preguntas.append(
    Pregunta(4, '1) ¿Cual es  la representación textual de un algoritmo?', 'b',
             2))
preguntas.append(Pregunta(5, '2) ¿Que es un algoritmo?', 'a', 2))
preguntas.append(
    Pregunta(
        6,
        '3) Son bloques o simbolos que representan cada una de las etapas de un algoritmo, siguiendo estandares que permita a cualquier persona comprenderlos',
        'd', 2))

preguntas.append(
    Pregunta(
        7,
        '1) La carpeta creada por Python utilizada para almacenar archivos pyc se llama :',
        'c', 3))
preguntas.append(
    Pregunta(
        8,
        '2) Es un operador capaz de verificar si dos valores son diferentes :',
        'b', 3))
...
alternativas.append(
    Alternativa(
        17,
        'a) Un conjunto de instrucciones o reglas bien definidas y ordenadas que permiten realizar una actividad mediante pasos sucesivos.',
        5))
alternativas.append(
    Alternativa(
        18,
        'b) Es una igualdad entre dos expresiones algebraicas, denominadas miembros, en las que aparecen valores conocidos o datos.',
        5))
alternativas.append(
    Alternativa(
        19,
        'c) Es una relación de variables que pueden ser cuantificadas para calcular el valor de otras de muy difícil calculo.',
        5))
...
alternativas.append(
    Alternativa(23, 'c) Simbologia de Diagramas de Entidad Relacion', 6))
...
alternativas.append(
    Alternativa(33, 'a) Se utiliza para determinar la longitud', 9))
alternativas.append(
    Alternativa(34, 'b) Se utiliza para separar una cadena ', 9))
alternativas.append(
    Alternativa(
        35,
        'c) Se utiliza para devolver un objeto que no tiene características',
        9))
alternativas.append(
    Alternativa(36,
                'd) Se utiliza para poner en mayuscula todos los caracteres',
                9))
...
def main():
    ...
    while True:
        ...
        if len(respuestaNombre) <= 0:
            print("Tu nombre debe tener al menos un caracter.")
            continue
          
        if not (bool(re.match("^[A-Za-z]+$", respuestaNombre))):
            print("Tu nombre debe tener sólo letras.")
            continue
        break
    ...
    while True:
        ...
        if sum(puntaje) > sum(puntajeBot):
            print('\n' + GREEN + 'Felicitaciones!! Ganaste con', sum(puntaje),
                  ' puntos y tu rival perdió con', sum(puntajeBot), 'puntos',
                  RESET)
        elif sum(puntaje) == sum(puntajeBot):
            print('\n' + YELLOW + 'Empataron con', sum(puntaje), 'puntos',
                  RESET)
        else:
            print('\n' + RED + 'Lo siento. Perdiste con', sum(puntaje),
                  ' puntos y tu rival ganó con', sum(puntajeBot), 'puntos',
                  RESET)

        respuestaReintentar = input(
            'Escribe Sí para reiniciar el juego, cualquier otra palabra para finalizar : '
        )
        print()
        if (respuestaReintentar.lower() != 'si'):
            print(
                "Juego finalizado. Si quieres reintentarlo, ejecuta el juego nuevamente."
            )
            break
